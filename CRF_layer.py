...
class constraint_CRF(nn.Module):
    def __init__(self, tagset_size):
        super(constraint_CRF, self).__init__()
        self.tagset_size = tagset_size
        # Matrix of transition parameters. Entry i,j is the score of transitioning from j to i. We have 2 additional TAGs which are START_TAG and STOP_TAG
        self.transitions = nn.Parameter(torch.rand(self.tagset_size, self.tagset_size))
    ...
    '''def _viterbi_decode(self, feats, constraint_list):
        backpointers = []      
        # Initialize the viterbi variables in log space
        init_vvars = torch.Tensor(1, self.tagset_size).fill_(0.)
        ...
        for i, feat in enumerate(feats):
            if i == 0:  # the first timestep, transfer from START_TAG to other stag. There is no need to reored the backpointers for first time step, since the first tag must be START_TAG           
                if constraint_list[i] > 0: #explicit
                    next_tag = argmax(feat.view(1,-1))
                    forward_var = torch.Tensor(1, self.tagset_size).fill_(-10000.)
                    forward_var[0,next_tag] = (feat[next_tag] + self.transitions_start[next_tag]).data[0] 
                    forward_var = autograd.Variable(forward_var)
                    if use_cuda:
                        forward_var = forward_var.cuda()
                else:
                    forward_var = forward_var + self.transitions_start.view(1, -1) + feat.view(1,-1)
                ...
                if constraint_list[i] > 0: #explicit
                    ...
                    best_tag_id = argmax(next_tag_var)
                    backpointers.append([best_tag_id] * (self.tagset_size))

                    forward_var = torch.Tensor(1, self.tagset_size).fill_(-10000.)
                    forward_var[0, next_tag] = (next_tag_var[0][best_tag_id] + feat[next_tag]).data[0] 
                    forward_var = autograd.Variable(forward_var)
                    if use_cuda:
                        forward_var = forward_var.cuda()
                else:                      #implicit
    ...
    def _viterbi_decode(self, feats, constraint_list):
        ...
        for i, feat in enumerate(feats):
            if i == 0:  # the first timestep, transfer from START_TAG to other stag. There is no need to reored the backpointers for first time step, since the first tag must be START_TAG           
                if constraint_list[i] < 0: # first timestep is implicit
                    forward_var = forward_var + self.transitions_start.view(1, -1) + feat.view(1,-1)
                else: #first timestep is explicit
                    next_tag = argmax(feat.view(1,-1))
                    forward_var = torch.Tensor(1, self.tagset_size).fill_(-10000.)
                    forward_var[0,next_tag] = (feat[next_tag] + self.transitions_start[next_tag]).data[0] 
                    forward_var = autograd.Variable(forward_var)
                ...
                if constraint_list[i] < 0 and constraint_list[i-1] > 0: #explicit-implicit
                #if constraint_list[i] < 0 and (constraint_list[i-1] > 0 or (i < len(constraint_list) -1 and constraint_list[i+1] > 0)): #explicit-implicit or implicit-explicit
                    bptrs_t = [] # holds the backpointers for this step
                    viterbivars_t = [] # holds the viterbi variables for this step
                    for next_tag in xrange(self.tagset_size):
                        # next_tag_var[i] holds the viterbi variable for tag i at the previous step plus the score of transitioning from tag i to next_tag.
